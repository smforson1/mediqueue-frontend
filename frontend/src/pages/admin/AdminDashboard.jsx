...
const AdminDashboard = ({ onLogout }) => {
  ...
  return (
    <div className="flex bg-[#f4f7fb] min-h-screen font-sans">
      ...
      {/* Sidebar */}
      <div className="w-64 bg-[#1b253b] text-slate-300 flex flex-col shadow-xl z-10">
        <div className="p-6">
          ...
          <nav className="space-y-1">
            <Link to="/admin" className="flex items-center gap-3 bg-blue-500/10 text-white border-l-4 border-blue-500 px-4 py-3 rounded-r-lg font-medium">
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="p-10 max-w-7xl mx-auto">
          ...
          <header className="flex justify-between items-start mb-8">
            <div>
              <h1 className="text-3xl font-black text-slate-900 mb-1">Dashboard Overview</h1>
              <p className="text-slate-500 font-medium tracking-tight">Manage patient queues and requests efficiently.</p>
            </div>
            <button className="bg-white border border-slate-200 text-slate-600 px-4 py-2.5 rounded-lg flex items-center gap-2 text-sm font-semibold shadow-sm hover:bg-slate-50 transition-colors">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm flex flex-col justify-between">
              ...
              <div>
                <div className="text-4xl font-black text-blue-600 mb-2">{stats?.waiting || 0}</div>
            ...
            <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm flex flex-col justify-between">
              ...
              <div>
                <div className="text-4xl font-black text-blue-600 mb-2">{stats?.appts || 0}</div>
            ...
            <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm flex flex-col justify-between">
              ...
              <div>
                <div className="text-4xl font-black text-[#1b253b] mb-2">{stats?.cancelled.toString().padStart(2, '0') || '00'}</div>
                <div className="flex items-center gap-1.5 text-sm text-slate-500 font-medium">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden p-6">
              <div className="flex justify-between items-center mb-6">
                ...
                <button className="text-blue-600 text-sm font-bold flex items-center hover:opacity-80">
                  View All &rarr;
                </button>
              </div>
                  ...
                  <tbody>
                    {queue.map((pat, idx) => (
                      <tr key={pat.id} className="border-b border-slate-50 last:border-0 hover:bg-slate-50/50">
                        <td className="py-5 font-bold text-slate-800 text-sm">{pat.name}</td>
                        <td className="py-5 text-slate-500 text-sm font-medium">{pat.time}</td>
                        <td className="py-5">
            ...
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 flex flex-col">
              ...
              <div className="space-y-4 flex-1">
                {pendingRequests.length === 0 ? (
                  <div className="text-center text-slate-500 py-8 text-sm">No pending requests at this time.</div>
                ) : (
